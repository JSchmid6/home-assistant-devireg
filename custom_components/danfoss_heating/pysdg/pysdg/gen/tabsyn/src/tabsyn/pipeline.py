...
from pysdg.gen.tabsyn.src.tabsyn.model.modules import (
    MLPDiffusion,
    Model,
)
from pysdg.gen.tabsyn.src.tabsyn.model.utils import sample
from pysdg.gen.tabsyn.src.tabsyn.model.vae import (
    Decoder_model,
    Encoder_model,
    Model_VAE,
)
from pysdg.gen.tabsyn.src.tabsyn.utils import (
    recover_data,
...
class TabSyn:
    ...
    def train_vae(self, max_beta, min_beta, lambd, num_epochs, save_path):
        ...
        for epoch in range(num_epochs):
            ...
            self.vae_model.eval()
            with torch.no_grad():
                test_num = self.X_test_num.to(self.device)
                test_cat = self.X_test_cat.to(self.device)
    ...
    def sample(
        ...
        syn_df = recover_data(syn_num, syn_cat, syn_target, info) #SMK: EXIT HERE

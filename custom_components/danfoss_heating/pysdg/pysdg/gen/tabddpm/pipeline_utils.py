...
from pysdg.gen.tabddpm.scripts.train import Trainer
...
from pysdg.gen.tabddpm.lib import (
    Transformations,
...
def load_multi_table(data_dir, verbose=True):
    ...
    for table, meta in dataset_meta["tables"].items():
        ...
        df_no_id = tables[table]["df"].drop(columns=id_cols) # SMK SMK Keep for now, but may be theres is NO need to drop id since data is already encoded. 
...
class Classifier(nn.Module): # SMK - This is the  NN architetcure that is used in the training function (it results in ouput predictions of dimension d_out whihc is likely to repressent labels). It has only the forward pass method but also inherits all methods from pytorch.
    def __init__(
        self,
        d_in,
        d_out,
...
def train_classifier(
    ...
    dataset, label_encoders, column_orders = make_dataset_from_df(
        ...
        df_info=df_info,
        std=0,
    ) # SMK - Here we prepare  and split the dataset into (train, val and test) based on teh given ratios
    print(dataset.n_features) 
    train_loader = prepare_fast_dataloader(
        dataset, split="train", batch_size=batch_size, y_type="long"
    )
    ...
    # SMK - Here we define the classifier acrhitecture 
    ...
    # SMK - Here we set the classdifer NN in training mode
    classifier.train()
    resume_step = 0
    
    # SMK - Training starts here
    for step in range(classifier_steps):
        ...
        classifier_optimizer.step() # SMK - This performs single optimization step
